...
describe('Rest Store', ()=>{
	beforeEach(()=>{
		...
		fetchMock.mock('http://localhost:3000/grocery?_page=1', ()=> mockData.grocery ) 
		fetchMock.mock( 'http://localhost:3000/grocery/41fd4fd9-95c7-4809-96db-a147d352fdbb', 100 )
	})

	afterEach(()=>fetchMock.restore())

	it( 'should retrieve data', async ()=>{
		const items = await DataStore.instance.getItems(1)
	...
	it( 'should update data', async ()=>{
		const item = ( await DataStore.instance.getItems(1))[0]
		item.favorite = false
		item.stock = 0

		await DataStore.instance.updateItem( item )
